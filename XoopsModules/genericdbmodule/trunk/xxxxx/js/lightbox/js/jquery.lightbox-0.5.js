...
(function($) {
	...
	$.fn.lightBox = function(settings) {
		...
		settings = jQuery.extend({
			...
			// Don��申alter these variables in any way
			imageArray:				[],
			activeImage:			0
		},settings);
		...
		function _start(objClicked,jQueryMatchedObj) {
			...
			// We have an image set? Or just an image? Let��申see it.
		...
		/**
		 * Prepares image exibition; doing a image��申preloader to calculate it��申size
		 *
		 */
		 ...
		 * Perfomance an effect in the image container resizing it
		 *
		 * @param integer intImageWidth The image��申width that will be showed
		 * @param integer intImageHeight The image��申height that will be showed
		 */
		function _resize_container_image_box(intImageWidth,intImageHeight) {
			...
			var intWidth = (intImageWidth + (settings.containerBorderSize * 2)); // Plus the image��申width and the left and right padding value
			var intHeight = (intImageHeight + (settings.containerBorderSize * 2)); // Plus the image��申height and the left and right padding value
			// Diferences
		...
		function _set_navigation() {
			...
			// Instead to define this configuration in CSS file, we define here. And it��申need to IE. Just.
		...
		function _keyboard_action(objEvent) {
			...
			if ( ( key == settings.keyToPrev ) || ( keycode == 37 ) ) {
				// If we��申 not showing the first image, call the previous
				if ( settings.activeImage != 0 ) {
					settings.activeImage = settings.activeImage - 1;
					_set_image_to_view();
			...
			if ( ( key == settings.keyToNext ) || ( keycode == 39 ) ) {
				// If we��申 not showing the last image, call the next
				if ( settings.activeImage != ( settings.imageArray.length - 1 ) ) {
					settings.activeImage = settings.activeImage + 1;
					_set_image_to_view();
					_disable_keyboard_navigation();
